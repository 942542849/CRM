...
@Repository
public interface AchieveClient_Dao {
	...
	public List<AchieveClient> select(@Param("txt") String txt,@Param("limit") String limit);
	...
	@Select("select c_client.*,c_clienttype.name clienttypename,c_operator.name createoperatorname,c_src.name srcname from ((c_client inner join c_clienttype on c_client.clienttype_id=c_clienttype.id) inner join c_operator on c_client.createoperator_id=c_operator.id) inner join c_src on c_client.src_id=c_src.id where (c_client.operatorids != '' and c_client.operatorids is not null and c_client.execstatu=2)${txt} ${limit}")
	public List<AchieveClient> Achieve(@Param("txt") String txt,@Param("limit") String limit);
//	
